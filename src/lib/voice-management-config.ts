import { getEnvConfigDisplayName, loadOptionalEnvFile } from "./env-file";
...
export function getVoiceManagementRuntime(): VoiceManagementRuntime {
  const localConfigFileName = "voice.env.local";
  ...
  const openApiAccessKey =
    process.env.VOLCENGINE_SPEECH_OPENAPI_ACCESS_KEY ?? localConfig.VOLCENGINE_SPEECH_OPENAPI_ACCESS_KEY ?? "";
  const openApiSecretKey =
    process.env.VOLCENGINE_SPEECH_OPENAPI_SECRET_KEY ?? localConfig.VOLCENGINE_SPEECH_OPENAPI_SECRET_KEY ?? "";
  const openApiProjectName =
    process.env.VOLCENGINE_SPEECH_OPENAPI_PROJECT_NAME ?? localConfig.VOLCENGINE_SPEECH_OPENAPI_PROJECT_NAME ?? "";
  ...
  const cloneResourceId =
    process.env.VOLCENGINE_VOICECLONE_RESOURCE_ID ?? localConfig.VOLCENGINE_VOICECLONE_RESOURCE_ID ?? "seed-icl-2.0";
  const defaultCloneSpeakerId =
    process.env.VOLCENGINE_VOICECLONE_DEFAULT_SPEAKER_ID ?? localConfig.VOLCENGINE_VOICECLONE_DEFAULT_SPEAKER_ID ?? "";
  ...
  return {
    ...
    openApiRegion:
      process.env.VOLCENGINE_SPEECH_OPENAPI_REGION ?? localConfig.VOLCENGINE_SPEECH_OPENAPI_REGION ?? "cn-beijing",
    openApiService:
