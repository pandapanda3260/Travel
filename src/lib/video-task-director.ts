...
function buildFallbackStoryShot(input: {
  ...
  return {
    ...
    backupAssetIds: input.planItem?.backupAssetIds,
    ...
    needsAiFallback: input.planItem?.needsAiFallback ?? false,
    fallbackReason: input.planItem?.fallbackReason ?? null,
    preRollSeconds: input.planItem?.preRollSeconds ?? 0,
    postRollSeconds: input.planItem?.postRollSeconds ?? 0,
    transitionType: input.planItem?.transitionType ?? input.planItem?.structure?.transitionType ?? null,
    negativePrompt: input.planItem?.negativePrompt ?? null,
    userOverride: input.planItem?.userOverride ?? false,
      ...
      restoreRealPhotoNarrationFieldsForShot(
        {
          ...
          preRollSeconds: shot.preRollSeconds ?? 0,
          postRollSeconds: shot.postRollSeconds ?? 0,
          transitionType: shot.transitionType ?? shot.structure?.transitionType ?? null,
          negativePrompt: shot.negativePrompt ?? null,
          userOverride: shot.userOverride ?? false,
