...
export default function ProductInfoPage() {
  ...
  const [runtime, setRuntime] = useState<ProductArchiveRuntime | null>(null);
  ...
  useEffect(() => {
    const loadPageData = async () => {
      ...
      try {
        const response = await fetch("/api/product-archives", { cache: "no-store" });
        const data = (await response.json()) as ProductArchivesPayload;
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    setSelectedArchiveId((current) => (current && archives.some((item) => item.archiveId === current) ? current : archives[0].archiveId));
  }, [archives]);
    ...
    () =>
      selectedArchive
        ? [
            ...
            { label: "解析模型", value: runtime ? `${runtime.providerLabel} / ${runtime.modelId}` : "未加载" },
          ]
        : [],
    [runtime, selectedArchive],
  );
  ...
  const insightStatusMeta = getArchiveModuleStatusMeta({
    hasArchive: Boolean(selectedArchive),
    hasImage: Boolean(selectedArchive?.sourceImageUrl),
    hasParsedText: Boolean(selectedArchive?.parsedData.tags.length || selectedArchive?.parsedData.sellingPoints.length || selectedArchive?.keyInfo.packagePersonCount),
  });

  function updateArchiveLocally(archiveId: string, updater: (current: ProductArchiveRecord) => ProductArchiveRecord) {
    setArchives((current) => sortArchivesByCreatedAtDesc(current.map((item) => (item.archiveId === archiveId ? updater(item) : item))));
  }
  ...
  async function handleCreateArchive() {
    ...
    try {
      ...
      setArchives((current) => sortArchivesByCreatedAtDesc([data.archive!, ...current.filter((item) => item.archiveId !== data.archive!.archiveId)]));
  ...
  async function handleUploadImage(file: File) {
    ...
    try {
      ...
      const response = await fetch(`/api/product-archives/${selectedArchive.archiveId}/image`, {
        method: "POST",
        body: formData,
      });
      const data = (await response.json()) as { archive?: ProductArchiveRecord; error?: string };
      ...
      updateArchiveLocally(selectedArchive.archiveId, () => data.archive!);
    } catch (uploadError) {
          ...
          <div className="dashboard-grid generation-tasks-grid product-archive-dashboard">
                ...
                <table className="task-table jobs-table">
                  ...
                  <tbody>
                    {archives.map((archive) => {
                      ...
                      return (
                      <tr key={archive.archiveId} className={archive.archiveId === selectedArchive?.archiveId ? "task-table-row-active" : ""}>
                        ...
                        <td className="submitted-time-cell">
                          ...
                          <strong>{new Date(archive.createdAt).toLocaleTimeString("zh-CN", { hour12: false })}</strong>
                        </td>
                        <td>
                          <div className="table-actions">
                            <button className="btn-pill" type="button" onClick={() => setSelectedArchiveId(archive.archiveId)}>
                              查看
                            </button>
                            <button className="btn-pill btn-pill-danger" type="button" disabled={deletingArchiveId === archive.archiveId} onClick={() => void handleDeleteArchive(archive.archiveId)}>
                    ...
                    {!archives.length ? (
                      <tr>
                        <td colSpan={5}>
                          <div className="product-archive-empty">还没有商品档案，先点击右上角按钮创建一条新的商品档案。</div>
                        </td>
                      </tr>
                    ) : null}
                  </tbody>
            ...
            <section className="panel preview-panel dashboard-preview product-archive-preview-panel">
              <ModuleTitle
                ...
                eyebrow="结果预览"
                level="primary"
                action={<span className="table-meta">{selectedArchive ? `当前档案：${getDisplayArchiveTitle(selectedArchive.title)}` : "未选择"}</span>}
              />
            ...
            <ModuleTitle
              ...
              action={
                <span className="table-meta">
                  {selectedArchive ? selectedArchive.archiveId.slice(0, 8) : "未选择"} · {runtime ? `${runtime.providerLabel} · ${runtime.liveEnabled ? "可调用" : "未启用"}` : "未加载"}
                </span>
              }
            />
                  ...
                  <div className="task-create-layout single-column product-archive-module-layout">
                    <div className="task-create-main">
                      <div className="product-archive-upload-row">
                        <button className="btn-primary task-next-step-button" type="button" disabled={isUploading} onClick={() => fileInputRef.current?.click()}>
                        ...
                        <label className="setting-field">
                          ...
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              updateArchiveLocally(selectedArchive.archiveId, (current) => ({ ...current, keyInfo: nextKeyInfo }));
                              scheduleArchiveSave(selectedArchive.archiveId, { keyInfo: { originalPrice: event.target.value } }, "originalPrice", { path: "keyInfo", previousValue: selectedArchive.keyInfo });
                            }}
                            placeholder="仅支持手动填写"
                          />
                        </label>
                        <label className="setting-field">
                          ...
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              updateArchiveLocally(selectedArchive.archiveId, (current) => ({ ...current, keyInfo: nextKeyInfo }));
                              scheduleArchiveSave(selectedArchive.archiveId, { keyInfo: { redeemPrice: event.target.value } }, "redeemPrice", { path: "keyInfo", previousValue: selectedArchive.keyInfo });
                            }}
                            placeholder="仅支持手动填写"
                          />
                        </label>
                        <label className="setting-field">
                          ...
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              updateArchiveLocally(selectedArchive.archiveId, (current) => ({ ...current, keyInfo: nextKeyInfo }));
                              scheduleArchiveSave(selectedArchive.archiveId, { keyInfo: { packagePersonCount: nextValue } }, "packagePersonCount", { path: "keyInfo", previousValue: selectedArchive.keyInfo });
                            }}
                ...
                <section className="composer-card voice-section-card inner-card">
                  ...
                  <section className="composer-card task-editor-card plain">
                    ...
                    <textarea
                      ...
                      onChange={(event) => {
                        const nextValue = event.target.value;
                        updateArchiveLocally(selectedArchive.archiveId, (current) => ({ ...current, parsedText: nextValue }));
                        scheduleArchiveSave(selectedArchive.archiveId, { parsedText: nextValue }, "parsedText", { path: "parsedText", previousValue: selectedArchive.parsedText });
                      }}
                ...
                <section className="composer-card voice-section-card inner-card">
                  ...
                  <div className="module-page-grid product-archive-info-grid">
                    <div className="module-page-card product-archive-info-card">
                      <span>基础信息</span>
                      <strong>{selectedArchive.keyInfo.productName || selectedArchive.parsedData.summaryTitle || "待识别"}</strong>
                      <p>{selectedArchive.parsedData.rawText ? "已识别图片文案，可继续人工修订。" : "上传图片后自动识别商品核心文案。"}</p>
                    </div>
                    <div className="module-page-card product-archive-info-card">
                      <span>价格与人数</span>
                    ...
                    <div className="module-page-card product-archive-info-card">
                      <span>识别标签</span>
                      <strong>{selectedArchive.parsedData.tags.length ? `${selectedArchive.parsedData.tags.length} 个标签` : "暂无标签"}</strong>
                      <p>{selectedArchive.parsedData.tags.join(" / ") || "上传图片后自动生成标签。"}</p>
                    </div>
                    <div className="module-page-card product-archive-info-card">
                      <span>卖点摘要</span>
                      <strong>{selectedArchive.parsedData.sellingPoints.length ? `${selectedArchive.parsedData.sellingPoints.length} 条卖点` : "暂无卖点"}</strong>
